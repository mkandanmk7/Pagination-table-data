async function page(set, get) {
  ...
  const fullDetails = await data.json(); // convertion of json to js.
...
function tableHead() {
  const table_head = document.createElement("table"); //table created
...
function previous() {
  if (n !== 0) {
    numbers[n - 1](); //call previous
  }
}
function next() {
  if (n !== 10) {
    numbers[n + 1](); //call next
  }
}
